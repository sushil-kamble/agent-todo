...
export function createTask(t) {
  const max = db
    ...
    .get(t.column_id).m
  db.prepare(
    `INSERT INTO tasks (id, title, project, agent, tag, column_id, position, created_at, mode, model, effort, fast_mode)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    t.id,
    t.title,
...
export function updateTaskFields(id, updates) {
  ...
  db.prepare(
    `UPDATE tasks SET title=?, project=?, agent=?, tag=?, column_id=?, position=?, mode=?, model=?, effort=?, fast_mode=? WHERE id = ?`
  ).run(
    next.title,
    next.project,
    next.agent,
