...
describe("tasks db", () => {
  ...
  it("creates tasks at the next position within a column", () => {
    const first = harness.tasks.createTask(
      taskFactory({ id: "t-a", column_id: "todo" }),
    );
    const second = harness.tasks.createTask(
      taskFactory({ id: "t-b", column_id: "todo" }),
    );
    const done = harness.tasks.createTask(
      taskFactory({ id: "t-c", column_id: "done" }),
    );
  ...
  it("listTasks returns run_status for active runs only", () => {
    ...
    const tFinished = harness.tasks.createTask(
      taskFactory({ id: "t-finished" }),
    );

    harness.runs.createRun(
      runFactory(tActive.id, { id: "r-active", status: "running" }),
    );
    harness.runs.createRun(
      runFactory(tFinished.id, { id: "r-finished", status: "completed" }),
    );
  ...
  it("listTaskStatuses returns only requested ids with null for no active run", () => {
    ...
    const byId = Object.fromEntries(
      rows.map((row) => [row.id, row.run_status]),
    );
